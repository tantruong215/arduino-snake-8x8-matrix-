...
// extern const PROGMEM bool snakeMessage[8][56];
// extern const PROGMEM bool gameOverMessage[8][90];
// extern const PROGMEM bool scoreMessage[8][58];
// extern const PROGMEM bool digits[][8][8];
...
void showTrophy(){
  for(int i=0;i<3;i++){
    ...
    for(int r=0;r<8;r++) for(int c=0;c<8;c++) matrix.setLed(0,r,c,1);
    delay(200);
    matrix.clearDisplay(0);
    delay(200);
  }
...
void generateFood(){
  if(food.row<0){
    if(snakeLength>=64){ win=true; return; }
    do{
      food.col = random(8);
      food.row = random(8);
...
void fixEdge(){
  if(currentMode==WALL_HIT){
    ...
    snake.col=(snake.col+8)%8; snake.row=(snake.row+8)%8;
  }
}
...
void calculateSnake(){
  ...
  fixEdge(); if(gameOver) return;
  matrix.setLed(0,snake.row,snake.col,1);
  if(gameboard[snake.row][snake.col]>1 && snakeDirection){ gameOver=true; return; }
  if(snake.row==food.row && snake.col==food.col){
    food={-1,-1}; snakeLength++;
    ...
    for(int r=0;r<8;r++) for(int c=0;c<8;c++) if(gameboard[r][c]>0) gameboard[r][c]++;
  }
  gameboard[snake.row][snake.col]=snakeLength+1;
  for(int r=0;r<8;r++) for(int c=0;c<8;c++){
    if(gameboard[r][c]>0){ gameboard[r][c]--; matrix.setLed(0,r,c,gameboard[r][c]?1:0); }
  }
}
...
void handleGameStates(){
  if(gameOver||win){
    ...
    if(score>highScore){ highScore=score; EEPROM.write(HI_SCORE_ADDR,highScore); showTrophy(); }
    scrollBitmap(gameOver?gameOverMessage:scoreMessage, gameOver?90:58);
    resetGame();
  }
}
...
void initialize(){
  ...
  matrix.shutdown(0,false); matrix.setIntensity(0,Pin::intensity); matrix.clearDisplay(0);
  randomSeed(analogRead(A5)); snake.row=random(8); snake.col=random(8);
}

void setup(){
  Serial.begin(115200);
  initialize(); calibrateJoystick();
  highScore=EEPROM.read(HI_SCORE_ADDR);
  showModeSelect(); scrollBitmap(snakeMessage,56);
}
...
void loop(){
  if(digitalRead(Pin::buttonPin)==LOW){
    delay(50);
    if(digitalRead(Pin::buttonPin)==LOW){
      paused=!paused; showPausedScreen(paused);
      if(paused){
        long t0=millis();
        while(digitalRead(Pin::buttonPin)==LOW){
          if(millis()-t0>2000){ resetGame(); paused=false; break; }
        }
      }
    }
  }
  if(paused) return;
  generateFood(); scanJoystick(); calculateSnake(); handleGameStates();
}
