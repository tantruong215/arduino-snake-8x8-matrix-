...
const char* modeNames[numModes] = { "1:WRAP", "2:WALL", "3:SPEED", "4:REVRS" };
...
// ---------------------- PROGMEM bitmaps ------------------
// (paste your original snakeMessage, gameOverMessage, scoreMessage, digits arrays here)
...
void initialize(){
  ...
  matrix.shutdown(0, false); matrix.setIntensity(0, Pin::intensity);
  matrix.clearDisplay(0); randomSeed(analogRead(A5));
  snake.row = random(8); snake.col = random(8);
}

void setup(){
  Serial.begin(115200); initialize(); calibrateJoystick();
    ...
    if(digitalRead(Pin::buttonPin)==LOW){
      ...
      if(paused){
        ...
        while(digitalRead(Pin::buttonPin)==LOW){
          if(millis()-st>2000){ resetGame(); paused=false; break; }
        }
      }
    }
  }
